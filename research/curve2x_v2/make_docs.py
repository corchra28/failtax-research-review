...
OUT="research/curve2x_v2"; R=json.load(open(f"{OUT}/results.json")); SPEC=json.load(open(f"{OUT}/frozen_spec.json")); BM=json.load(open(f"{OUT}/build_manifest.json")); LC=json.load(open(f"{OUT}/label_check.json")); TR=json.load(open(f"{OUT}/test_results.json"))
...
m=EV["VAL+CONF"][N]; arm={k:v for k,v in (m.get('all_rows_metrics') or {}).items() if k not in ('rel_tp','rel_sl')}; card+=["",f"- Orizonturi secundare (0,25 SOL, VAL+CONF): 5M {m.get('horizon_5M')}; 30M {m.get('horizon_30M')}",f"- Calibrare in regiunea tranzactionata (VAL+CONF, 0,25): {m.get('traded_region_calibration')}",f"- Baseline STATE_HEADROOM (M0) cu aceeasi politica (VAL+CONF, 0,25): {m.get('baseline_M0_same_policy')}",f"- Metrici pe toate randurile VAL+CONF (0,25): {arm}",
 "",f"## Porti PAPER_CANDIDATE (regula 28)","| poarta | rezultat |","|---|---|"]+FUNNEL_+[f"| {k} | {'PASS' if v else 'FAIL'} |" for k,v in g.items()]+["",f"**FINAL_VERDICT = {R['FINAL_VERDICT']}**; READY_FOR_REAL_MONEY = NO; LIVE_TRADING_ENABLED = NO.","",
 ...
 f"- Automatizare: replay-only; AUTOMATION_REPLAY_AGREEMENT = {RC.get('AUTOMATION_REPLAY_AGREEMENT')} (batch {RC.get('batch_candidates')} vs replay {RC.get('replay_candidates')})."]
...
lines=[f"CURVE2X_V2 | {R['FINAL_VERDICT']} | model {sel[0]}/{sel[1]} | policy {pol} | model_hash {R['model_hash'][:16]}..",f"VAL   0.25 SOL: mints {v.get('usable',0)} TP {f(v.get('TP_FIRST_rate'),3)} SL {f(v.get('SL_FIRST_rate'),3)} EV {f(v.get('EV'))} PF {f(v.get('PF'),2)} CI95 {v.get('CI95')}",f"CONF  0.25 SOL: mints {c_.get('usable',0)} TP {f(c_.get('TP_FIRST_rate'),3)} SL {f(c_.get('SL_FIRST_rate'),3)} EV {f(c_.get('EV'))} PF {f(c_.get('PF'),2)} CI95 {c_.get('CI95')}",f"TOTAL 0.25 SOL: mints {a_.get('usable',0)} EV {f(a_.get('EV'))} PF {f(a_.get('PF'),2)} CI95 {a_.get('CI95')} EXB1 {f(a_.get('EX_BEST_1PCT'))} stress+5 {f(summ['STRESS_5_SLOT_EV'])} cost+25% {f(summ['STRESS_COST125_EV'])}",f"0.50 SOL EV {f(summ['EV_050_SOL'])} | 1.00 SOL EV {f(summ['EV_100_SOL'])}","GATES: "+", ".join(f"{k}={'PASS' if x else 'FAIL'}" for k,x in g.items())]
