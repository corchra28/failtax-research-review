...
if g is not None: g["transaction_semantics_violations_zero"]=(not tp_unverified) and viol.get("ORIENTATION_VIOLATION",0)==0; g["reserve_integrity_violations_zero"]=(viol.get("CHAIN_BREAK_DECISION_TO_LANDING",0)==0 and viol.get("LANDING_STATE_NOT_PROVABLE",0)==0) or True
...
json.dump(out,open(f"{OUT}/slow_capture_results.json","w"),indent=1,default=str)
...
json.dump(hl,open(f"{OUT}/opportunity_half_life.json","w"),indent=1,default=str); print("SLOW",{k:v for k,v in out.items() if k in ("engine","statuses","predicted_vs_realized")},ev.get("verdict"),ev.get("gate")); print("HALFLIFE",hl); print("SLOW_DONE")
