...
N=collections.Counter(); out=gzip.open(f"{D}/census_candidates.jsonl.gz","wt"); t0=time.time()
for fp in sorted(glob.glob(f"{TAPE}/events_*.jsonl.gz")):
    for line in readlines(fp):
        ...
        for d in evs:
            ...
            groups[(d["user"],tok)].append(d)
        for (u,tok),g in groups.items():
            pools={d["pool"] for d in g}
            if len(pools)<2 or not any(d["is_buy"] for d in g) or not all(d["is_buy"] or True for d in g) or not any(not d["is_buy"] for d in g): continue
            N["candidate_groups"]+=1
            out.write(json.dumps(dict(sig=r["sig"],slot=r["slot"],t=r["t"],user=u,token=tok,n_events_in_tx=len(r["events"]),n_swaps_in_tx=len(evs),unknown_events_in_tx=unknown,duplicate_event_keys=dupflag,users_in_tx=len({d["user"] for d in evs}),events=g),separators=(",",":"))+"\n")
    print(os.path.basename(fp),dict(N),round(time.time()-t0),"s",flush=True)
out.close(); json.dump(dict(built=time.strftime("%Y-%m-%d %H:%M:%S"),counters=dict(N),cache_sha256=hashlib.sha256(open(f"{D}/census_candidates.jsonl.gz","rb").read()).hexdigest(),script_sha256=hashlib.sha256(open(__file__,"rb").read()).hexdigest(),pools_in_metadata=len(META)),open(f"{D}/census_scan_manifest.json","w"),indent=1); print("CENSUS_SCAN_DONE",flush=True)
