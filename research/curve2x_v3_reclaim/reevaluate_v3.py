...
unav_orig=ODM["splice_unavailable"]; unav_new=DM["splice_unavailable"]; repaired=unav_orig-unav_new   # 'reparat' = splice-unavailable in original cu CompleteEvent DUPA orizont (acum CURVE_ONLY)
diff=dict(splice_unavailable_original=unav_orig,splice_unavailable_remediated=unav_new,repaired_by_horizon_fix=repaired,migrated_in_window_original=ODM["migrated"],migrated_in_window_remediated=DM["migrated"],splice_ok_original=ODM["splice_ok"],splice_ok_remediated=DM["splice_ok"],bounds_status=DM.get("bounds_status"),entry_positions_hist=DM.get("bounds_entry_positions_hist"),
