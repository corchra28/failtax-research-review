...
## 7. Închiderea ipotezei primare și noua ipoteză SLOW_ATOMIC_REVERT_ARB (revizuirea 4; fără RPC, fără PnL)
- ATOMIC_ARB_PRIMARY_MEME = **INSUFFICIENT_EXISTING_DATA_UNIQUE_TOKEN_GATE** (3 token-uri < 20; praguri și rezultate păstrate în research/atomic_same_mint_arb_populations_frozen.json). Căile vechi stage_run/stage_freeze (pair-by-pair, dedup doar pe (notional, slot), legate de vechiul feasibility) sunt dezactivate în cod.
- SLOW_ATOMIC_REVERT_ARB (research/slow_atomic_revert_arb_frozen_spec.json): studiu general de arbitraj multipool între pool-uri noncanonice pentru același token, exclusiv pe SECONDARY_ALL_NONCANONICAL (13 token-uri, 80 pool-uri, 477 combinații, 3.807 ferestre curate (token, slot), 3 zile UTC); proiectat fără a inspecta spread-uri, prețuri sau PnL; nu salvează ipoteza primară.
- Calea reală de execuție (run_engine + stage_run_slow): populația secundară înghețată + maparea RPC + evenimentele pass 2; rutele unui token = toate perechile ordonate; selecția = token_episode_selection integrată în motor (max o decizie per token și episod, ruta exclusiv după predicted, reset doar când toate rutele revin la ≤ 0; combinațiile de perechi nu sunt observații independente; cheia portofoliului = (token, episod) + max o tranzacție per slot); VQ estimat doar din evenimentele ≤ decizie (fără lookahead).
- Model de infrastructură înghețat: Helius WSS/RPC obișnuit, fără Jito; decizie din ultima stare completă observabilă; landing primar s+3, stres s+5; ambele leg-uri într-o tranzacție atomică, leg-ul 2 cu gardă min_out = Q + taxa de semnătură + prioritate; gardă picată ⇒ REVERTED (fără pierdere de inventar, taxa completă 0,000105 SOL); reușite și revertate raportate separat; taxe noncanonical 25/5/0; niciodată supply hardcodat.
- Porți economice înghețate înainte de orice PnL: ≥ 50 episoade independente la nivel de token, ≥ 5 token-uri, EV > 0 după costurile tranzacțiilor reușite și revertate, PF ≥ 1,5, CI95 (cluster token × zi UTC) > 0, PnL pozitiv în ≥ 2 din 3 zile, ex-best-1 % > 0, niciun token > 40 % din profitul brut, EV fără rutele USDC > 0, EV la s+5 > 0.
- Teste comportamentale (research/slow_atomic_revert_arb_tests.py, date sintetice): motorul apelează selecția pe episoade (spy), 3 pool-uri / 6 rute ⇒ o singură tranzacție per episod și un al doilea episod doar după reset, pool-ul cu orientare inversă exclus fără duplicate, prima tranzacție nu apare înaintea slotului dislocării (fără lookahead), predicted-ul reconstruit doar din starea de decizie, starea viitoare nu schimbă predicted-ul, modelul de revert aplicat când garda pică la landing, loader-ul citește SECONDARY din fișierul înghețat, stage_run_slow consumă populația înghețată, calea veche este închisă.
- Mint RPC: pregătit, NEEXECUTAT, restrâns la cele 13 token-uri secundare (1 apel, câmpuri owner/mint_authority/supply/decimals/initialized/freeze_authority, scop și SHA-256 în research/mint_metadata_recovery_manifest.json; gardă MINT_RPC_APPROVED).
