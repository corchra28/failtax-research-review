...
stage=sys.argv[1]
if stage=="freeze":
    mints=sorted(json.load(open(POP))["SECONDARY_ALL_NONCANONICAL"]["tokens"].keys()); assert len(mints)<=BATCH*MAX_CALLS, "peste bugetul de 2 apeluri"   # NUMAI cele 13 token-uri secundare
    lst="\n".join(mints); open(f"{D}/mint_list_frozen.txt","w").write(lst)
    man=dict(frozen_at=time.strftime("%Y-%m-%d %H:%M:%S %Z"),source_commit=subprocess.run(["git","rev-parse","HEAD"],capture_output=True,text=True).stdout.strip(),n_mints=len(mints),mint_list_sha256=hashlib.sha256(lst.encode()).hexdigest(),batch_size=BATCH,n_batches=(len(mints)+BATCH-1)//BATCH,max_calls=MAX_CALLS,rpc_method="getMultipleAccounts",params=dict(commitment="finalized",encoding="base64",dataSlice=dict(offset=0,length=SLICE)),layout="mint_authority option(4+32)@0 supply u64@36 decimals u8@44 is_initialized u8@45 freeze_authority option(4+32)@46",purpose="SLOW_ATOMIC_REVERT_ARB: excluderea Token-2022/owner necunoscut si validarea supply/decimals/initialized/authorities pentru cele 13 token-uri ale populatiei SECONDARY_ALL_NONCANONICAL; fara PnL",fields=["account.owner","mint_authority(option)","supply(u64)","decimals(u8)","is_initialized(u8)","freeze_authority(option)"],mints=mints,requires="MINT_RPC_APPROVED=YES (aprobare explicita)",script_sha256=hashlib.sha256(open(__file__,"rb").read()).hexdigest(),status="FROZEN_NOT_EXECUTED (fetch cere aprobare explicita)")
